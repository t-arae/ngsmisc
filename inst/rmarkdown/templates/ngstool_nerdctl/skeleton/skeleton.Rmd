---
title: 'Rmd template for works using external commands via nerdctl on lima'
...
<!--START templates/ngstool_nerdctl header-->
```{css, echo=F}
.scroll_dark {
  max-height: 300px;
  overflow-y: auto;
...
```
<!--END templates/ngstool_nerdctl header-->

<!--START templates/ngstool_nerdctl body_1-->
...
# Common setup
## lima + nerdctl command
```{r cmd_lima_nerd}
nerdctl <- "/opt/homebrew/bin/limactl shell default nerdctl"
cmd_lima_nerd <- "{nerdctl} run --rm -v {wd}:/data -v {shared}:/config"
```
...
```
<!--END templates/ngstool_nerdctl body_1-->

<!--START templates/ngstool_nerdctl body_2-->
# Container
...
# samtools example
"{nerdctl} run --rm {img_name} samtools --version" %>%
  stringr::str_glue() %>%
...
# samtools example
"{nerdctl} run --rm {img_name} samtools --help" %>%
  stringr::str_glue() %>%
  ngsmisc::run_cat_stdout()
```
<!--END templates/ngstool_nerdctl body_2-->

<!--START templates/ngstool_nerdctl body_3-->
# Command run
...
out <-
  stringr::str_glue(cmd_lima_nerd, img_name, "bash temp.sh", .sep = " ") %>%
...
```
<!--END templates/ngstool_nerdctl body_3-->

<!--START templates/ngstool_nerdctl footer-->
# Sessioninfo
```{r sessioninfo}
sessionInfo()
```
<!--END templates/ngstool_nerdctl footer-->
